...
import {
  Loader, Heart, ShoppingBag, ArrowLeft,
  ChevronDown, X, AlertTriangle, CheckCircle,
} from "lucide-react";

// ── Filter options  (match Myntra dataset values) ─────────────────────────
...
const OCCASION_OPTIONS = ["All Occasions","Casual","Party","Formal","Sports","Ethnic"];
const CATEGORY_OPTIONS = [
  "All Categories",
  "Tshirts","Shirts","Tops","Blouses","Sweatshirts","Jackets","Blazers",
  "Kurtas","Tunics","Dresses","Sarees","Jeans","Trousers","Shorts","Skirts","Leggings",
];
...
function isColorSuitable(color, skinTone) {
  ...
  if (!ok) return true;
  return ok.includes(color.toLowerCase().trim());
}
    ...
    <div className="relative">
      <button
        ...
        className={`w-full flex items-center justify-between px-3 py-2.5 rounded-xl transition-all font-semibold text-sm border-2
          ${colorUnsafe ? "bg-red-50 border-red-300 text-red-700"
            : isActive  ? "bg-purple-50 border-purple-400 text-purple-800"
                        : "bg-gray-50 border-gray-200 text-gray-700 hover:bg-gray-100"}`}
      >
        <span className="flex items-center gap-2 truncate">
          <span>{emoji}</span>
          <span className="truncate">{value}</span>
          {colorUnsafe && <AlertTriangle className="w-3.5 h-3.5 text-red-500 flex-shrink-0" />}
        </span>
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            {options.map((option) => {
              const dotClass  = id === "color" ? COLOR_DOT_MAP[option.toLowerCase()] : null;
              ...
              return (
                <button key={option}
                  ...
                  className={`w-full text-left px-3 py-2.5 text-sm flex items-center gap-2 transition-colors
                    ${isSelected ? "bg-purple-100 font-bold text-purple-800"
                      : isUnsafe  ? "hover:bg-red-50 text-gray-700"
...
export default function RecommendationsPage() {
  ...
  useEffect(() => {
    ...
    const fetchRecs = async () => {
      setLoading(true); setError(null); setFilters(INITIAL_FILTERS);
      try {
        const payload = {
        ...
        console.log("📤 Recommend payload:", payload);

        const res = await axios.post("http://127.0.0.1:8000/recommend/generate", payload);
  ...
  const resetFilters = useCallback(() => {
    setFilters(INITIAL_FILTERS); setOpenDropdown(null);
  }, []);
        ...
        <div className="mb-6">
          <div className="flex items-center gap-4 mb-6">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 mt-0.5">
                ...
                clothes for{" "}
                <span className="font-semibold text-purple-600">{selectedDetails.body_type}</span> body &amp;{" "}
                <span className="font-semibold text-pink-600">{selectedDetails.skin_tone}</span> skin
              </p>
            </div>
          ...
          <div className="grid grid-cols-4 gap-3 mb-6">
            ...
            <div className="bg-white rounded-2xl shadow-sm p-4 border-l-4 border-purple-400">
              ...
              <p className="text-base font-bold text-purple-600 mt-0.5">{selectedDetails.body_type}</p>
            </div>
            <div className="bg-white rounded-2xl shadow-sm p-4 border-l-4 border-pink-400">
              ...
              <p className="text-base font-bold text-pink-600 mt-0.5">{selectedDetails.skin_tone}</p>
            </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm p-4 mb-6 border border-gray-100"
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-2">
              <FilterDropdown id="color"    emoji="🎨" options={COLOR_OPTIONS}    value={filters.color}    onChange={handleFilterChange} openDropdown={openDropdown} setOpenDropdown={setOpenDropdown} skinTone={skinTone} />
              <FilterDropdown id="sleeve"   emoji="👕" options={SLEEVE_OPTIONS}   value={filters.sleeve}   onChange={handleFilterChange} openDropdown={openDropdown} setOpenDropdown={setOpenDropdown} skinTone={skinTone} />
              <FilterDropdown id="occasion" emoji="🎉" options={OCCASION_OPTIONS} value={filters.occasion} onChange={handleFilterChange} openDropdown={openDropdown} setOpenDropdown={setOpenDropdown} skinTone={skinTone} />
              <FilterDropdown id="category" emoji="👗" options={CATEGORY_OPTIONS} value={filters.category} onChange={handleFilterChange} openDropdown={openDropdown} setOpenDropdown={setOpenDropdown} skinTone={skinTone} />
            </div>
          ...
          <motion.div layout
            ...
            <AnimatePresence mode="popLayout">
              {filtered.map((outfit, index) => {
                ...
                const dotClass     = COLOR_DOT_MAP[outfit.color?.toLowerCase()] || "bg-gray-300";
                    ...
                    <div className="relative h-48 overflow-hidden">
                      ...
                      {colorUnsafe && (
                        <div className="absolute bottom-0 left-0 right-0 bg-red-500/90 text-white px-2 py-1 flex items-center gap-1">
                          ...
                          <span className="text-xs font-semibold truncate">Not ideal for {skinTone} skin</span>
                        </div>
                      )}

                      <button onClick={() => toggleWishlist(outfit)} disabled={isProcessing}
                        className={`absolute right-2 rounded-full p-2 shadow-md transition-all
                          ${colorUnsafe ? "bottom-8" : "bottom-2"}
                          ${isProcessing ? "bg-gray-100 text-gray-400 cursor-wait"
                            : inWishlist  ? "bg-red-500 text-white scale-110"
                    ...
                    <div className="p-3">
                      ...
                      {colorUnsafe && (
                        <div className="flex items-center gap-1 mb-2 px-1.5 py-1 bg-red-50 border border-red-200 rounded-lg">
                          ...
                          <span className="text-xs text-red-600 font-medium">Not ideal for your skin tone</span>
                        </div>
                      )}
